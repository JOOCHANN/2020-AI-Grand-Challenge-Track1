...
def invert_affine(preds):
    for i in range(len(preds)):
        if len(preds[i]['rois']) == 0:
            continue
        else:
            preds[i]['rois'][:, [0, 2]] = preds[i]['rois'][:, [0, 2]] / (1280 / 960)
            preds[i]['rois'][:, [1, 3]] = preds[i]['rois'][:, [1, 3]] / (720 / 540)
    return preds
...
def postprocess(x, anchors, regression, classification, regressBoxes, clipBoxes, threshold, iou_threshold):
    ...
    # anchors_tmp= transformed_anchors[:,:,2:4]-transformed_anchors[:,:,:2]
    # keep =anchors_tmp[:,:,0]>anchors_tmp[:,:,1]
    # transformed_anchors= transformed_anchors[keep]
    # print(transformed_anchors.shape)
    # transformed_anchors=transformed_anchors.view(-1,transformed_anchors.size(0),4)
    # classification= classification[keep]
    # classification=classification.view(-1,classification.size(0),90)
    ...
    for i in range(x.shape[0]):
        ...
        transformed_anchors_per = transformed_anchors[0, scores_over_thresh[i, :], ...]
